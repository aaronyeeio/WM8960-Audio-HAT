...
if [ "x${is_Radxa}" != "xRadxa" ] ; then
  echo "Sorry, this drivers only works on Radxa ZERO 3"
  exit 1
fi
...
apt update
apt-get -y install linux-headers-vendor-rk35xx #raspberrypi-kernel 
...
# install dtbos
armbian-add-overlay wm8960-soundcard.dts
